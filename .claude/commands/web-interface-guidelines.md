---
description: Review UI code against Vercel's Web Interface Guidelines (accessibility, interactions, performance, design)
---

Review the UI code for compliance with the Web Interface Guidelines below.

Scope: $ARGUMENTS
(If no scope is given, review the current branch diff / recently changed UI files.)

For each relevant file, report violations grouped by severity using the
guideline's own MUST / SHOULD / NEVER language. Cite `file:line` for each
finding and give a concrete fix. End with a short prioritized checklist. Do not
restate rules that already pass — only surface gaps.
