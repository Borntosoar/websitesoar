/* SOAR — 3D entrance for the Shopify password page (vanilla Three.js).
   White studio; a black box trembles; on submit the lid blows off and a BIRD
   soars up and OUT of the box, wings beating, scattering the box as it goes —
   black & white. Server validates the password while the animation plays.
   Progressive: if WebGL/CDN is unavailable the static SVG gate remains. */
...
(function () {
  ...
  var dir = new THREE.DirectionalLight(WHITE, 0.9);
  ...
  var shell = new THREE.Mesh(shellGeo, new THREE.MeshStandardMaterial({ color: 0x0a0a0a, roughness: 0.5, metalness: 0.1, transparent: true, opacity: 0.9 }));
  boxGroup.add(shell);
  boxGroup.add(new THREE.LineSegments(new THREE.EdgesGeometry(shellGeo), new THREE.LineBasicMaterial({ color: 0x000000 })));

  var lid = new THREE.Group();
  var lidGeo = new THREE.BoxGeometry(1.72, 0.06, 1.72);
  lid.add(new THREE.Mesh(lidGeo, new THREE.MeshStandardMaterial({ color: 0x0a0a0a, transparent: true, opacity: 0.9 })));
  lid.add(new THREE.LineSegments(new THREE.EdgesGeometry(lidGeo), new THREE.LineBasicMaterial({ color: 0x000000 })));
  lid.position.y = 0.88;
  boxGroup.add(lid);

  var innerLight = new THREE.PointLight(0xffffff, 1.8, 6, 1.8);
  innerLight.position.y = 0;
  boxGroup.add(innerLight);

  /* ---- subtle studio floor ---- */
  var grid = new THREE.GridHelper(16, 32, 0xe2e2e2, 0xefefef);
  grid.position.y = -0.9;
  rig.add(grid);
  ...
  var wingMat = new THREE.MeshStandardMaterial({ color: 0x080808, roughness: 0.55, side: THREE.DoubleSide, transparent: true, opacity: 1 });
  ...
  /* ---- box fragments (scatter as the bird breaks out) ---- */
  var SPARKS = 24, sparks = [];
  var sparkGeo = new THREE.BoxGeometry(0.07, 0.07, 0.07);
  for (var i = 0; i < SPARKS; i++) {
    var sm = new THREE.MeshStandardMaterial({ color: 0x0a0a0a, roughness: 0.6, transparent: true, opacity: 0 });
  ...
  /* ---- white glow (light flooding from the box) ---- */
  var glowTex = (function () {
    var c = document.createElement("canvas"); c.width = c.height = 128;
    var ctx = c.getContext("2d");
    var g = ctx.createRadialGradient(64, 64, 0, 64, 64, 64);
    g.addColorStop(0, "rgba(255,255,255,0.95)"); g.addColorStop(0.4, "rgba(255,255,255,0.25)"); g.addColorStop(1, "rgba(255,255,255,0)");
    ctx.fillStyle = g; ctx.fillRect(0, 0, 128, 128);
    var t = new THREE.CanvasTexture(c); t.colorSpace = THREE.SRGBColorSpace; return t;
  })();
  var glow = new THREE.Sprite(new THREE.SpriteMaterial({ map: glowTex, transparent: true, opacity: 0, depthWrite: false, blending: THREE.AdditiveBlending }));
  glow.position.y = 0.4; glow.scale.set(5, 5, 1);
  rig.add(glow);
