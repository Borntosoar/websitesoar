...
/**
 * Campaign blocks — full-bleed editorial tiles in the rhythm of premium
 * streetwear homepages (campaign hero -> category split), with SOAR's own
 * content and gradient "photography" placeholders (swap for real campaign
 * imagery when shot).
 */
...
function CampaignBlock({
  ...
  return (
    <section className="relative h-[88svh] w-full overflow-hidden text-bone" style={{ background: bg }}>
      <div
        aria-hidden
      ...
      >
        ...
        <Reveal delay={80}>
          <h2 className="max-w-3xl text-4xl font-medium leading-[1.02] tracking-tight md:text-6xl">
            {title}
          </h2>
        </Reveal>
        <Reveal delay={160}>
...
/* Split category tiles — two-up, full-bleed */
const tiles = [
  ...
  return (
    <section className="grid md:grid-cols-2">
      {tiles.map((t, i) => (
        ...
        >
          <span className="font-serif text-[9rem] italic leading-none text-ink/[0.07] absolute right-6 top-2 select-none transition-transform duration-700 group-hover:-translate-y-2">
            {i === 0 ? "I" : "II"}
          </span>
