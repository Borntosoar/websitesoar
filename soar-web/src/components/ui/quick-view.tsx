...
function Panel({ product }: { product: Product }) {
  ...
  function addToBag() {
    ...
    for (let i = 0; i < qty; i++) {
      cart.add({ id: product.id, handle: product.handle, variantId: product.variantBySize?.[size || product.sizes[0]], title: product.name, price: product.price, category: product.category, size: size || undefined, image: product.image });
    }
    close();
  }
